jQuery(document).ready(function($){
    ...
    var datatable = $('#dataTable').DataTable( {
        ...
        },
        {
            ...
            render: function (data, type, row, meta) {
                return `
                    <a href="`+site_url+`/roles/${row.id}/edit" data-id="${row.id}" class="w-32-px h-32-px bg-success-focus text-success-main rounded-circle d-inline-flex align-items-center justify-content-center">
                  <iconify-icon icon="lucide:edit"></iconify-icon>
                </a>
                 <a href="javascript:void(0)"  data-id="${row.id}" class="w-32-px h-32-px bg-danger-focus text-danger-main rounded-circle d-inline-flex align-items-center justify-content-center delete-role">
